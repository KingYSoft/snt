...
import { queryPortCodePage, deletePortCode } from '@/service/api/maintain/port-code';
import { getPortCodeColumns } from './modules/columns';
import EditDrawer from './modules/edit-drawer.vue';

defineOptions({ name: 'PageMaintainPortCode' });
...
const {
  ...
  queryFn: params => queryPortCodePage(params),
  deleteFn: deletePortCode,
  getColumns: (editCb, deleteCb) => getPortCodeColumns(editCb, deleteCb),
  defaultSearchKey: 'rl_code'
});

const searchKeyOptions = [
  { label: 'Port Code', value: 'rl_code' },
  { label: 'Port Name', value: 'rl_port_name' }
];
const opOptions = [
...
<template>
  <div class="h-full flex-col-stretch gap-16px overflow-hidden">
    <NCard :title="$t('route.maintain_port-code')" :bordered="false" class="flex-shrink-0">
