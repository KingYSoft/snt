...
import { savePackageType } from '@/service/api/maintain/package-types';
...
const defaultFormData = () => ({ id: 0, code: '', desc: '', is_active: true });
...
const rules: FormRules = { code: { required: true, message: $t('form.required') } };

async function handleSubmit() {
  await formRef.value?.validate();
  loading.value = true;
  try {
    await savePackageType(mapFrontendToBackend(formData.value, { zeroOneFields: ['is_active'] }));
  ...
  <NDrawer v-model:show="visible" :width="480">
    <NDrawerContent :title="title" :native-scrollbar="false">
      <NForm ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100">
        <NFormItem :label="$t('page.maintain.packageTypes.code')" path="code">
          <NInput v-model:value="formData.code" />
        </NFormItem>
        <NFormItem :label="$t('page.maintain.packageTypes.description')">
          <NInput v-model:value="formData.desc" type="textarea" />
        </NFormItem>
        <NFormItem :label="$t('page.maintain.packageTypes.isActive')">
