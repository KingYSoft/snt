...
import { queryPackageTypePage, deletePackageType } from '@/service/api/maintain/package-types';
import { getPackageTypeColumns } from './modules/columns';
import EditDrawer from './modules/edit-drawer.vue';

defineOptions({ name: 'PageMaintainPackageTypes' });
...
const { data, loading, columns, pagination, getData, getDataByPage } = useNaivePaginatedTable<any, any>({
  api: async () => {
    return queryPackageTypePage({
      ...buildSjcPaginationParams(pageRef.value, pageSizeRef.value),
      filters: buildFilters()
    });
  },
  columns: () => getPackageTypeColumns((id: any) => editRef.value(id), (row: any) => deleteRef.value(row)),
...
async function handleDelete(row: any) {
  ...
  try {
    await deletePackageType(row.id);
...
<template>
  <div class="h-full flex-col-stretch gap-16px overflow-hidden">
    <NCard :title="$t('route.maintain_package-types')" :bordered="false" class="flex-shrink-0">
