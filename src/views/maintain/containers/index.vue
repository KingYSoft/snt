...
import { queryContainerTypePage, deleteContainerType } from '@/service/api/maintain/containers';
import { getContainerTypeColumns } from './modules/columns';
import EditDrawer from './modules/edit-drawer.vue';

defineOptions({ name: 'PageMaintainContainers' });
...
const { data, loading, columns, pagination, getData, getDataByPage } = useNaivePaginatedTable<any, any>({
  api: async () => {
    return queryContainerTypePage({
      ...buildSjcPaginationParams(pageRef.value, pageSizeRef.value),
      filters: buildFilters()
    });
  },
  columns: () => getContainerTypeColumns((id: any) => editRef.value(id), (row: any) => deleteRef.value(row)),
...
async function handleDelete(row: any) {
  ...
  try {
    await deleteContainerType(row.id);
...
<template>
  <div class="h-full flex-col-stretch gap-16px overflow-hidden">
    <NCard :title="$t('route.maintain_containers')" :bordered="false" class="flex-shrink-0">
