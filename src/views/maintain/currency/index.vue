...
import { queryCurrencyPage, deleteCurrency } from '@/service/api/maintain/currency';
import { getCurrencyColumns } from './modules/columns';
import EditDrawer from './modules/edit-drawer.vue';

defineOptions({ name: 'PageMaintainCurrency' });
...
const { data, loading, columns, pagination, getData, getDataByPage } = useNaivePaginatedTable<any, any>({
  api: async () => {
    return queryCurrencyPage({
      ...buildSjcPaginationParams(pageRef.value, pageSizeRef.value),
      filters: buildFilters()
    });
  },
  columns: () => getCurrencyColumns((id: any) => editRef.value(id), (row: any) => deleteRef.value(row)),
...
<template>
  <div class="h-full flex-col-stretch gap-16px overflow-hidden">
    <NCard :title="$t('route.maintain_currency')" :bordered="false" class="flex-shrink-0">
