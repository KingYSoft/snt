...
const { t } = useI18n();
const te = (key: string) =>
  t(`page.settlement.matchTransactions.editor.${key}`);
...
async function handleSelectCompany(value: string) {
  ...
  if (selected?.data) {
    ...
    form.value.settleCompanyName =
      selected.data.name ?? selected.data.company_name ?? "";
...
const mapLine = (raw: any, index: number) => {
  ...
  const rawId =
    raw.id ?? raw.line_id ?? raw.lineId ?? raw.pk ?? `line-${index}`;
  return {
    id: String(rawId),
    ...
    settlement_amount_original:
      Number(
        raw.settlement_amount_original ?? raw.settlementAmountOriginal ?? 0,
      ) || 0,
    ex_rate: Number(raw.ex_rate ?? raw.exRate ?? 1) || 1,
    settlement_amount_home:
      Number(raw.settlement_amount_home ?? raw.settlementAmountHome ?? 0) || 0,
  };
};
...
async function queryOutstandingInvoiceLines(companyName: string) {
  ...
  if (statementVal.value?.trim())
    payload.statementNo = statementVal.value.trim();
  if (lineCurrency.value) payload.currency = lineCurrency.value;
  if (moreChargeDesc.value?.trim())
    payload.chargeDesc = moreChargeDesc.value.trim();
  ...
  allLines.value = sourceItems
    .map((item: any, idx: number) => mapLine(item, idx))
    .filter(Boolean);
  checkedLineKeys.value = [];
}
...
async function onSearchLines() {
  ...
  const companyName = String(
    form.value.settleCompany?.company_name ?? "",
  ).trim();
  if (!companyName) {
...
const selectedOutstandingTotal = computed(() =>
  selectedLines.value.reduce(
    (acc: number, row: any) => acc + (Number(row.outstanding) || 0),
    0,
  ),
);
...
const summaryTotalHomeAmount = computed(() =>
  summaryRows.value.reduce(
    (acc: number, r: any) => acc + (Number(r.homeAmount) || 0),
    0,
  ),
);
...
watch(
  ...
  ([outstandingTotal, settleAmount]) => {
    form.value.balance =
      (Number(settleAmount) || 0) - (Number(outstandingTotal) || 0);
  },
  { immediate: true },
);
...
async function handleSave() {
  ...
  const sorted = [...selectedLines.value].sort(
    (a: any, b: any) =>
      (Number(a.outstanding) || 0) - (Number(b.outstanding) || 0),
  );
  ...
  try {
    const res: any = await matchTransactionsSaveMatchWriteOff({
      ...
      bankAccountId: String(
        form.value.bankAccount?.id ?? form.value.bankAccount?.pk ?? "",
      ),
...
onMounted(async () => {
  ...
  try {
    ...
    if (items.length > 0) {
      const first = items[0];
      form.value.matchNumber = String(
        first.matchNumber ?? first.match_number ?? "",
      );
      form.value.settleCompanyName = String(
        first.tth_billing_party ?? first.billingParty ?? "",
      );
      ...
      form.value.chequeNo = String(
        first.tth_invoice_payment_reference_code ?? "",
      );
      ...
      form.value.settleAmount = items.reduce(
        (s: number, x: any) =>
          s + (Number(x.tth_ts_total ?? x.settlementAmountOriginal ?? 0) || 0),
        0,
      );
    }
  } catch (error) {
      ...
      <template v-else>
          ...
          <NGi :span="6">
            <NSpace vertical>
              <div class="flex items-center gap-8px">
                <span class="shrink-0 w-80px text-right text-12px"
                  >{{ te("settleCompany") }}:</span
                >
                <NSelect
                  :value="
                    form.settleCompany?.pk ?? form.settleCompany?.id ?? ''
                  "
              ...
              <NGrid :cols="2" :x-gap="12" :y-gap="8">
                <NGi>
                  <div class="flex items-center gap-8px">
                    ...
                    <NSelect
                      :value="
                        form.bankAccount?.id ?? form.bankAccount?.pk ?? ''
                      "
                      :options="bankOptions"
                      :loading="bankLoading"
                      filterable
                ...
                <NGi>
                  <NInput
                    v-model:value="form.refNo"
                    :disabled="editorLocked"
                    :placeholder="te('pleaseInput')"
                    ><template #prefix>{{ te("refNo") }}:</template></NInput
                  >
                </NGi>
                <NGi>
                  <NInput
                    ...
                    :disabled="editorLocked"
                    :placeholder="te('pleaseInput')"
                    ><template #prefix>{{ te("chequeNo") }}:</template></NInput
                  >
                </NGi>
                <NGi>
                  <NInputNumber
                ...
                <NGi>
                  <NInput
                    ...
                    ><template #prefix>{{ te("balance") }}:</template></NInput
                  >
                </NGi>
              </NGrid>
            </NCard>
                ...
                <table class="w-full text-12px">
                  ...
                  <tbody>
                    <tr v-for="r in summaryRows" :key="r.key">
                      ...
                      <td class="text-right pa-4px">
                        {{ r.currency }} {{ formatNum(r.exRate, 6) }}
                      </td>
                      <td class="text-right pa-4px">
        ...
        <NSpace align="center" class="mb-8px py-8px" :wrap="true">
          ...
          <NInput
            ...
            :placeholder="te('pleaseInput')"
            style="width: 180px"
            ><template #prefix>{{ te("statementNo") }}:</template></NInput
          >
          <NSelect
