...
defineOptions({ name: 'PageSettlementPayableTransactionsCreate' });
...
function createEmptyLine(): ApChargeLine {
  return {
    id: generateNewId(),
...
function calcAmount(item: ApChargeLine) {
...
function calcHomeAmount(item: ApChargeLine) {
...
<template>
  <div class="h-full overflow-auto p-16px">
    <NCard title="New AP Transaction" :bordered="false">
      ...
      <NDataTable
        ...
        :bordered="true"
        size="small"
        :row-key="(row: ApChargeLine) => row.id"
        :scroll-x="2500"
        :max-height="400"
        striped
      />
