...
function mapBillingRecordToTransaction(item: any): SettlementTransactionRecord {
  ...
  const creditorDebtorCode =
    item.oh_e2_oa_address || item.oh_oa_code || item.ah_oh || "";
  const creditorDebtorName =
    item.oh_e2_companyname || item.oh_oa_name || item.ab_e2_companyname || "";
  ...
  const jobInvoiceNumber =
    item.ah_consolidatedinvoiceref ||
    item.ah_chequeorreference ||
    item.ah_jobnumber ||
    item.ah_transactionnum ||
    "";
...
export async function queryReceivableTransactions(
  ...
  if (response?.data?.items && Array.isArray(response.data.items)) {
    response.data.items = response.data.items
      ...
      .map(mapBillingRecordToTransaction);
  }

  return response;
}

export async function queryPayableTransactions(
  params: SettlementTransactionQueryParams,
) {
  const billingParams: BillingTblInput = {
    skipCount: params.skipCount,
